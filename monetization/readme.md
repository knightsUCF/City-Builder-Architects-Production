...
We could also sell items to sell inside the stores the players create. Perhaps those items would not give any sort of competitive advantage, but instead would be equal based on "class" of the other generic items the other players can sell, so the whales could have almost an infinite number of items to buy to sell, just to make their gameplay more unique, but on paper the items don't give you a competitive advantage. Somehow need to establish this. So perhaps show the stats of the locked items, so the players who will not spend, can compare the items, to make sure that gameplay is fair. The only tricky thing about this is we would want to make sure the player knows this part is not pay to play. If Clash of Titans is so bold aobut their pay to play to mechanisms, and other games, then who cares if the player assumes this, since this is a business model that works, and instead over time most players will realize this is not an advantage. they will be able to check the grayed out box for stats anyway. This will give them a unique style based on the items they sell, and there will be almost an infite supply of these items that we can keep adding after the game is published, like they kept adding the characters to Crossy Road. 
